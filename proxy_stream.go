...
// streamGoogleToChat 将 Google Gemini SSE 流转为 Chat Completions SSE 并写入 w
func (b *ProxyRuntime) streamGoogleToChat(w http.ResponseWriter, body io.Reader, model string) (int64, int64, int64) {
	flusher, ok := w.(http.Flusher)
	if !ok {
		return 0, 0, 0
	}
	...
	for reader.Scan() {
		...
		if deltaText != "" {
			contentText.WriteString(deltaText)
		}
		...
		if finishReason != "" {
			chatChunk["choices"].([]any)[0].(map[string]any)["finish_reason"] = finishReason
			chatChunk["usage"] = map[string]any{
